...
- **Artist-Only Access**: Requires artist site membership verified with `is_user_member_of_blog()` against `ec_get_blog_id( 'artist' )`
...
- WordPress multisite
- The extrachill theme (provides homepage and breadcrumb hooks)
- `ec_get_blog_id()` helper available (from `extrachill-multisite`) for artist site blog ID resolution

## Installation

This plugin is deployed as part of the Extra Chill Platform. Activation is site-specific on stream.extrachill.com.
...
- **Asset Loading**: `inc/core/assets.php` conditionally enqueues CSS/JS for the stream site and localizes artist data for vanilla JS
- **REST Placeholder**: `inc/core/rest-api.php` implements permissions and returns a placeholder response for stream status
...
- Build artifacts are created with `./build.sh` (symlinked to `/.github/build.sh`).
...
This plugin is maintained as part of the Extra Chill Platform.
</content>
